...
Assert-Contains $postInit 'PHEN_CS_fnc_CSS_getPIPImpactSolution' 'Aim prediction must centralize impact calculation through the PIP-only solution entrypoint.'
Assert-Contains $postInit 'PIP_fnc_updatePath' 'PIP-only bullet path must call the EBW PIP path updater.'
Assert-Contains $postInit 'PIP_fnc_proxyP0Dir' 'PIP-only bullet path must read the EBW PIP proxy origin/direction.'
Assert-Contains $postInit 'PIP_fnc_refreshProxyCache' 'PIP-only bullet path must refresh the EBW PIP proxy cache before solving.'
Assert-Contains $postInit 'PIP_fnc_firstValidHit' 'PIP-only bullet path must require the EBW PIP first-valid-hit helper.'
Assert-Contains $postInit 'PIP_fnc_accelACE' 'PIP-only bullet path must preserve the EBW PIP ACE acceleration branch.'
Assert-Contains $postInit 'PIP_cache_impactLocal' 'PIP-only bullet path must consume the EBW PIP impact-local cache result.'
Assert-Contains $postInit 'PIP_paramsACE' 'PIP-only bullet path must populate EBW PIP ACE parameter state.'
Assert-Contains $postInit 'PHEN_CS_fnc_CSS_tracePIP40mmGLProfile' 'Aim prediction must route supported 40mm grenade profiles through the PIP 40mm trajectory helper.'
Assert-Contains $postInit 'PHEN_CS_fnc_CSS_applyPIPScopeAdjustment' 'PIP 40mm trajectory must apply ACE scope adjustment before tracing.'
Assert-Contains $postInit 'ace_scopes_fnc_getBoreHeight' 'PIP bullet integration must preserve ACE bore-height input for the PIP solver.'
Assert-Contains $postInit 'ace_advanced_ballistics_fnc_readAmmoDataFromConfig' 'PIP bullet integration must feed ACE advanced-ballistics ammo data into PIP when available.'
...
Assert-Contains $postInit '"pip_unsupported_projectile"' 'Rocket and missile ammo must be rejected by the PIP-only solution entrypoint.'
Assert-Contains $postInit '"pip_unsupported_nonbullet"' 'Unsupported non-bullet ammo must be rejected by the PIP-only solution entrypoint.'
Assert-Contains $postInit '"pip_ebw_missing"' 'PIP-only prediction must report missing EBW PIP functions instead of falling back to local CSS ballistics.'
Assert-Contains $postInit '"pip_ebw_no_solution"' 'PIP-only prediction must record when the EBW PIP solver cannot produce a valid hit.'
Assert-Contains $postInit '"pip_40mm"' 'PIP-only prediction must label supported 40mm solutions as PIP 40mm.'
...
Assert-Contains $postInit '[PHEN_CS][ArgusAim]' 'Aim telemetry must have a compact RPT prefix.'
...
Assert-Contains $postInit 'PHEN_CS_fnc_CSS_logPostShotImpactDebug' 'Argus post-shot telemetry must emit compact impact telemetry that survives RPT line truncation.'
Assert-Contains $postInit '[PHEN_CS][ArgusShot]' 'Argus post-shot telemetry must have a distinct RPT prefix.'
Assert-Contains $postInit '[PHEN_CS][ArgusShotImpact]' 'Argus post-shot impact telemetry must have a compact RPT prefix.'
...
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_updateAimPrediction\s*=\s*\{[\s\S]*call\s+PHEN_CS_fnc_CSS_getPIPImpactSolution' 'Aim prediction must delegate impact calculation to the PIP-only solution entrypoint.'
Assert-NotRegex $postInit 'PHEN_CS_fnc_CSS_updateAimPrediction\s*=\s*\{[\s\S]*call\s+PHEN_CS_fnc_CSS_getShotCalibration[\s\S]*call\s+PHEN_CS_fnc_CSS_applyAimCalibration' 'Aim prediction must not use the old CSS shot-calibration loop.'
...
Assert-Contains $postInit '"pip_unsupported_projectile"' 'Rocket and missile ammo must be rejected by the PIP-only solution entrypoint instead of receiving an approximate CSS marker.'
...
Assert-Contains $postInit '"predictionMeta"' 'Post-shot trace telemetry must preserve PIP solution metadata.'
Assert-RuntimeAndPboContains '"pipUseACE"' 'Argus PIP telemetry must log whether the PIP ACE branch was selected.'
Assert-RuntimeAndPboContains '"pipParamsACEReady"' 'Argus PIP telemetry must log whether PIP ACE parameters were populated.'
Assert-RuntimeAndPboContains '"pipImpactLocal"' 'Argus PIP telemetry must log the EBW PIP local impact vector.'
Assert-RuntimeAndPboContains '"PIPI_2_EBW"' 'Argus PIP telemetry must identify the EBW PIP bullet source.'
Assert-RuntimeAndPboContains '"PIPI_EBW_40mm"' 'Argus PIP telemetry must identify the 40mm PIP source.'
...
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_getPIPImpactSolution\s*=\s*\{[\s\S]*"pip_unsupported_projectile"[\s\S]*"NO SOLUTION"' 'Unsupported launchers and missiles must record a visible PIP no-solution reason instead of drawing an approximate marker.'
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_getPIPImpactSolution\s*=\s*\{[\s\S]*isNil "PIP_fnc_updatePath"[\s\S]*"pip_ebw_missing"' 'PIP-only bullet prediction must fail closed when EBW PIP functions are unavailable.'
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_getPIPImpactSolution\s*=\s*\{[\s\S]*PIP_params\s*=[\s\S]*PIP_paramsACE\s*=[\s\S]*call PIP_fnc_updatePath[\s\S]*PIP_cache_impactLocal' 'PIP-only bullet prediction must configure PIP globals, call the EBW updater, then consume PIP_cache_impactLocal.'
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_getPIPImpactSolution\s*=\s*\{[\s\S]*ace_advanced_ballistics_fnc_readAmmoDataFromConfig[\s\S]*PIP_paramsACE' 'PIP-only bullet prediction must feed ACE advanced-ballistics data into the EBW PIP ACE parameter path.'
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_tracePIP40mmGLProfile\s*=\s*\{[\s\S]*while\s*\{\s*_steps <= 50000[\s\S]*lineIntersectsSurfaces[\s\S]*call PHEN_CS_fnc_CSS_getTrajectoryHitStatus' 'PIP 40mm tracing must integrate the copied 40mm profile until a validated surface hit or range limit.'
Assert-Regex $postInit 'PHEN_CS_CSS_AimSolution\s*=\s*\[_impactASL,\s*diag_tickTime \+ PHEN_CS_CSS_AimSolutionTTL,\s*_method,\s*_label,\s*_zeroDistance,\s*_meta\];' 'Aim prediction must write the PIP result directly without old cache or calibration wrappers.'
Assert-NotRegex $postInit 'PHEN_CS_fnc_CSS_updateAimPrediction\s*=\s*\{[\s\S]*"rocket_approx"[\s\S]*PHEN_CS_CSS_AimSolution\s*=' 'PIP-only aim prediction must not draw approximate CSS rocket markers.'
...
Assert-NotRegex $postInit 'private\s+_dir\s*=\s*vectorNormalized\s*\(_unit weaponDirection _weapon\);[\s\S]*private\s+_startASL\s*=\s*\(eyePos _unit\)' 'Aim prediction must not use the removed old eyePos/weaponDirection-only CSS solver path.'
...
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_onFiredDebug\s*=\s*\{[\s\S]*getShotInfo _projectile' 'Argus FiredMan telemetry must keep real shotInfo/projectile velocity diagnostics.'
Assert-NotRegex $postInit 'PHEN_CS_fnc_CSS_onFiredDebug\s*=\s*\{[\s\S]*call\s+PHEN_CS_fnc_CSS_storeShotCalibration' 'Argus FiredMan telemetry must not feed old shot calibration back into pre-shot prediction.'
Assert-Contains $postInit '"predictionMeta"' 'Argus FiredMan telemetry must expose the PIP solution metadata in debug state.'
...
Assert-Contains $postInit '"predictedImpactASL"' 'Argus fired and trace_end telemetry must log the PIP predicted impact point.'
Assert-Contains $postInit '"predictionMeta"' 'Argus fired and trace_end telemetry must log the PIP solution metadata.'
Assert-Contains $postInit '"postShotImpactPointASL"' 'Argus trace_end telemetry must log the post-shot impact point used for scoring.'
Assert-Contains $postInit '"predictionError"' 'Argus trace_end telemetry must log the PIP prediction error.'
Assert-Contains $postInit '"predictionErrorVectorASL"' 'Argus trace_end telemetry must log the PIP prediction error vector.'
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_logPostShotImpactDebug\s*=\s*\{[\s\S]*"predictedImpactASL"[\s\S]*"postShotImpactPointASL"[\s\S]*"predictionError"[\s\S]*"predictionErrorVectorASL"[\s\S]*"predictionErrorUsable"' 'Compact post-shot impact telemetry must log predicted impact, post-shot impact, prediction error vector, and usability before large prediction arrays can truncate the RPT line.'
...
Assert-NotContains $postInit 'PHEN_CS_CSS_AimDebugState = PHEN_CS_CSS_PostShotDebugState' 'Pre-shot and post-shot debug states must not alias each other. (source runtime)'
Assert-NotContains $postInit 'PHEN_CS_CSS_PostShotDebugState = PHEN_CS_CSS_AimDebugState' 'Pre-shot and post-shot debug states must not alias each other. (source runtime)'
Assert-NotContains $pboPayload 'PHEN_CS_CSS_AimDebugState = PHEN_CS_CSS_PostShotDebugState' 'Pre-shot and post-shot debug states must not alias each other. (packed PBO)'
Assert-NotContains $pboPayload 'PHEN_CS_CSS_PostShotDebugState = PHEN_CS_CSS_AimDebugState' 'Pre-shot and post-shot debug states must not alias each other. (packed PBO)'
...
Assert-Contains $pboPayload '[PHEN_CS][ArgusShotImpact]' 'Packed PBO must contain compact Argus post-shot impact telemetry.'
Assert-Contains $pboPayload 'getShotInfo _projectile' 'Packed PBO must contain real projectile shotInfo telemetry.'
Assert-Contains $pboPayload 'PHEN_CS_fnc_CSS_getPIPImpactSolution' 'Packed PBO must contain the PIP-only impact solution entrypoint.'
Assert-Contains $pboPayload 'PIP_fnc_updatePath' 'Packed PBO must contain the EBW PIP update-path integration.'
Assert-Contains $pboPayload 'PIP_cache_impactLocal' 'Packed PBO must contain EBW PIP impact-local cache consumption.'
Assert-Contains $pboPayload 'PIP_paramsACE' 'Packed PBO must contain EBW PIP ACE parameter integration.'
Assert-Contains $pboPayload 'PHEN_CS_fnc_CSS_tracePIP40mmGLProfile' 'Packed PBO must contain the PIP 40mm helper.'
Assert-Contains $pboPayload 'pip_unsupported_projectile' 'Packed PBO must reject unsupported rockets and missiles through PIP no-solution state.'
Assert-Contains $pboPayload 'pip_ebw_no_solution' 'Packed PBO must contain EBW PIP no-solution state.'
Assert-Contains $pboPayload 'PIPI_2_EBW' 'Packed PBO must identify the EBW PIP bullet source.'
Assert-Contains $pboPayload 'PIPI_EBW_40mm' 'Packed PBO must identify the 40mm PIP source.'
Assert-Contains $pboPayload 'impactEvaluation' 'Packed PBO must contain post-shot impact evaluation.'
Assert-Contains $pboPayload 'predictionMeta' 'Packed PBO must contain PIP prediction metadata telemetry.'
Assert-Contains $pboPayload 'predictedImpactASL' 'Packed PBO must contain PIP predicted impact telemetry.'
Assert-Contains $pboPayload 'postShotImpactPointASL' 'Packed PBO must contain post-shot impact point telemetry.'
Assert-Contains $pboPayload 'predictionErrorVectorASL' 'Packed PBO must contain PIP prediction error vector telemetry.'
