...
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_getShotCalibrationKey\s*=\s*\{[\s\S]*format \["%1\|%2\|%3\|%4\|%5\|%6", _weapon, _muzzle, _mode, _magazine, _ammo, _stanceName\]' 'Shot calibration key must be shared across optic zeroing distances for the same weapon/muzzle/mode/magazine/ammo/stance.'
...
Assert-Contains $pboPayload 'format ["%1|%2|%3|%4|%5|%6", _weapon, _muzzle, _mode, _magazine, _ammo, _stanceName]' 'Packed PBO must contain the zeroing-independent shot calibration key.'
