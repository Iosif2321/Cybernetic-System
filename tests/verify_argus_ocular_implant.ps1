...
Assert-Contains $postInit 'PHEN_CS_fnc_CSS_logAimCorrectionDebug' 'Aim prediction must emit compact correction telemetry that survives RPT line truncation.'
Assert-Contains $postInit '[PHEN_CS][ArgusAimCorrection]' 'Aim correction telemetry must have a compact RPT prefix.'
...
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_getShotCalibrationKey\s*=\s*\{[\s\S]*format \["%1\|%2\|%3\|%4\|%5\|shared_zeroing\|%6", _weapon, _muzzle, _mode, _magazine, _ammo, _stanceName\]' 'Shot calibration key must be shared across optic zeroing distances for the same weapon/muzzle/mode/magazine/ammo/stance.'
Assert-NotRegex $postInit 'PHEN_CS_fnc_CSS_getShotCalibrationKey\s*=\s*\{[\s\S]*round _zeroDistance' 'Shot calibration key must not include zeroDistance, or correction resets when optic zeroing changes.'
...
Assert-RuntimeAndPboContains '"aceDecision"' 'Argus telemetry must preserve ACE branch decision diagnostics.'
Assert-RuntimeAndPboContains '"aceEnabled"' 'Argus ACE diagnostics must log whether ACE advanced ballistics is enabled.'
Assert-RuntimeAndPboContains '"isACEAmmo"' 'Argus ACE diagnostics must log whether the ammo name was classified as ACE ammo.'
Assert-RuntimeAndPboContains '"hasACEConfig"' 'Argus ACE diagnostics must log whether ACE ballistic config fields were detected.'
Assert-RuntimeAndPboContains '"ammoCfgClass"' 'Argus ACE diagnostics must log the resolved ammo config class.'
Assert-RuntimeAndPboContains '"ammoCfgIsClass"' 'Argus ACE diagnostics must log whether the resolved ammo config is a class.'
Assert-RuntimeAndPboContains '"aceBallisticCoefficientsIsArray"' 'Argus ACE diagnostics must log whether ACE_ballisticCoefficients was visible as an array.'
Assert-RuntimeAndPboContains '"aceDragModelIsNumber"' 'Argus ACE diagnostics must log whether ACE_dragModel was visible as a number.'
Assert-RuntimeAndPboContains '"useACE"' 'Argus ACE diagnostics must log the final ACE branch decision.'
Assert-RuntimeAndPboContains '"aceDecisionReason"' 'Argus ACE diagnostics must log the first explicit reason ACE mode was not selected.'
...
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_updateAimPrediction\s*=\s*\{[\s\S]*"rocket_approx"[\s\S]*PHEN_CS_CSS_AimSolution\s*=\s*\[_aimCacheKey,\s*_aimInputDir,\s*_projectileFamily,\s*\[_hit # 0,\s*diag_tickTime \+ PHEN_CS_CSS_AimSolutionTTL,\s*"rocket"' 'Unguided shotrocket launchers must produce an approximate rocket aim solution instead of launcher_no_solution.'
...
Assert-Contains $postInit 'PHEN_CS_fnc_CSS_traceAimTrajectory' 'Aim prediction must use a shared trajectory tracer for comparable pre/post correction points.'
Assert-Contains $postInit '"correction"' 'Aim debug payload must expose correction telemetry.'
Assert-Contains $postInit '"preCorrectionAimPointASL"' 'Aim debug payload must expose the aim point before shot calibration correction.'
Assert-Contains $postInit '"postCorrectionAimPointASL"' 'Aim debug payload must expose the aim point after shot calibration correction.'
Assert-Contains $postInit '"correctionDistance"' 'Aim debug payload must expose the distance between pre- and post-correction aim points.'
Assert-Contains $postInit '"calibrationSharedAcrossZeroing"' 'Aim debug payload must show that shot calibration is shared across optic zeroing distances.'
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_logAimCorrectionDebug\s*=\s*\{[\s\S]*"preCorrectionAimPointASL"[\s\S]*"postCorrectionAimPointASL"[\s\S]*"correctionDeltaASL"[\s\S]*"correctionDistance"' 'Compact aim correction telemetry must log pre/post correction points and the correction delta before large debug arrays can truncate the RPT line.'
...
Assert-Contains $postInit '"preShotUncorrectedAimPointASL"' 'Argus fired and trace_end telemetry must log the pre-shot aim point before correction.'
Assert-Contains $postInit '"preShotCorrectedAimPointASL"' 'Argus fired and trace_end telemetry must log the corrected pre-shot aim point.'
...
Assert-Contains $postInit '"impactVsCorrectedError"' 'Argus trace_end telemetry must log impact-vs-corrected-point error.'
Assert-Contains $postInit '"impactVsCorrectedVectorASL"' 'Argus trace_end telemetry must log impact-vs-corrected-point vector.'
Assert-Regex $postInit 'PHEN_CS_fnc_CSS_logPostShotImpactDebug\s*=\s*\{[\s\S]*"preShotUncorrectedAimPointASL"[\s\S]*"preShotCorrectedAimPointASL"[\s\S]*"postShotImpactPointASL"[\s\S]*"impactVsCorrectedError"[\s\S]*"predictionErrorUsable"' 'Compact post-shot impact telemetry must log pre-shot points, post-shot impact, correction error, and usability before large prediction arrays can truncate the RPT line.'
...
Assert-Contains $pboPayload '[PHEN_CS][ArgusAimCorrection]' 'Packed PBO must contain compact Argus aim correction telemetry.'
...
Assert-Contains $pboPayload 'format ["%1|%2|%3|%4|%5|shared_zeroing|%6", _weapon, _muzzle, _mode, _magazine, _ammo, _stanceName]' 'Packed PBO must contain the zeroing-independent shot calibration key.'
Assert-NotContains $pboPayload 'round _zeroDistance' 'Packed PBO must not contain the old zeroing-specific shot calibration key.'
...
Assert-Contains $pboPayload 'PHEN_CS_fnc_CSS_traceAimTrajectory' 'Packed PBO must contain the shared pre/post correction trajectory tracer.'
Assert-Contains $pboPayload 'preShotUncorrectedAimPointASL' 'Packed PBO must contain pre-shot uncorrected aim point telemetry.'
Assert-Contains $pboPayload 'preShotCorrectedAimPointASL' 'Packed PBO must contain pre-shot corrected aim point telemetry.'
Assert-Contains $pboPayload 'postShotImpactPointASL' 'Packed PBO must contain post-shot impact point telemetry.'
Assert-Contains $pboPayload 'impactVsCorrectedError' 'Packed PBO must contain impact-vs-corrected-point error telemetry.'
Assert-Contains $pboPayload 'impactVsCorrectedVectorASL' 'Packed PBO must contain impact-vs-corrected-point vector telemetry.'
Assert-Contains $pboPayload 'calibrationSharedAcrossZeroing' 'Packed PBO must contain cross-zeroing calibration telemetry.'
