...
- 3D-метки показываются только для объектов, которые находятся на экране, перед камерой игрока и проходят проверку видимости;
- объекты за закрывающей геометрией не должны получать 3D-метку, если это отсекается проверкой `checkVisibility`;
- мини-радар союзных контактов с переключаемым масштабом и направлением контактов;
...
- live-маркер расчетной точки прицеливания до выстрела через текущую линию оружия и приблизительный баллистический расчет.

Argus больше не выдает игроку отдельное оружие-бинокль и не переключает оружие в руках. Обычная фокусировка взгляда Arma остается на штатной клавише игрока. Мод только читает состояние `zoomTemp`, чтобы HUD мог понимать, что игрок сейчас использует native focus.

При включенном ACE Advanced Ballistics маркер точки прицеливания остается приблизительным. ACE документирует расширенные параметры баллистики, но в публичной документации ACE нет подтвержденного solver API, который можно безопасно вызывать для точного предсказания траектории из этого мода.

## Управление Argus в CBA

Клавиша находится в категории:

```text
[FOD] Cybernetics System - Tactical HUD -> Cycle Argus Radar Scale
```

По умолчанию клавиша не назначена. При наличии `Argus Combat Optics Mk.IV` она переключает масштаб мини-радара. Доступные масштабы: 250, 500, 1000 и 2000 метров.
...
Не передавайте classnames экипировки, массивы шлемов или имена переменных без кавычек, если эти переменные уже не содержат валидные строки с ID киберимплантов или отображаемыми названиями киберимплантов. Например, этот вариант валиден только если каждый массив `HOV_MK*_helmets` содержит строки, совпадающие с ID киберимплантов или отображаемыми названиями киберимплантов:
